    ...
    ui(new Ui::Line)
{
    ...
    connect(ui->flag0, static_cast<void (QCheckBox::*)(int)>(&QCheckBox::stateChanged), [=](int state) {
        ...
        for(int x = 0; x < nodes.count(); x++)
            nodes[x]->setActive(parents->at(nodes[x]->getLine1())->isActive()&&parents->at(nodes[x]->getLine2())->isActive()&&mode==Crosscorrelator);
    });
...
void Line::setFlag(int flag, bool value)
{
    switch (flag) {
    case 0:
        ui->flag0->stateChanged(value << 1);
        break;
    case 1:
        ui->flag1->stateChanged(value << 1);
        break;
    case 2:
        ui->flag2->stateChanged(value << 1);
        break;
    case 3:
        ui->flag3->stateChanged(value << 1);
        break;
    default:
        break;
    }
...
void Line::stackCorrelations()
{
    ...
    stop = !isActive();
    ahp_xc_scan_autocorrelations(line, &spectrum, &stop, &percent);
    if(spectrum != nullptr) {
        ...
        stack += 1.0;
        for (int x = start; x < ahp_xc_get_delaysize(); x++) {
